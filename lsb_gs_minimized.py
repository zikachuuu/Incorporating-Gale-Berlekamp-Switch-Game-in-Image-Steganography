from gbSolver3 import GB_Solver3
import os
...
TECHNIQUE       = 1         # 0 for LSB Replacement, 1 for LSB Matching
WRITE_TO_CSV    = True      # True to write emedding result to csv, False otherwise
START_PAYLOAD   = 19         # inclusive
END_PAYLOAD     = 20        # exclusive
...
for payload in range (START_PAYLOAD , END_PAYLOAD) :
    ...
    for t in range (1 , 10) :
        ...
        with open(SCRIPT_PATH + f'\\random bit sequences\\bit_sequence_payload-{payload}-{t}.txt' , 'r') as file:
        ...
        optimizedMatrix, optimizedBitOneCount = GB_Solver3 (bit_sequence_matrix , columnSwitch, rowSwitch, dim)
        ...
        imageio.imwrite (f"{SCRIPT_PATH}/results/image_{'lsbr' if TECHNIQUE == 0 else 'lsbm'}_gs_minimized/{IMAGE_NAME[:-4]}_{payload}-{t}.png" , stego)

        if WRITE_TO_CSV :
            writeEmbeddingResultToCSV (IMAGE_NAME, f"bit_sequence_payload-{payload}-{t}.txt", payload, len(bit_sequence), originalBitChanges, optimizedBitChange, endTime - startTime)
